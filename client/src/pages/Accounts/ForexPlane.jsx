...
import { ForexPlans } from '../../constants/InvestmentData'
...
const ForexPlane = () => {
  return (
    ...
    <HelmetProvider>
      <Helmet>
        <title>Forex Plan - SwizzFunds</title>
    ...
    <div className="black-gradient  py-10 sm:py-20 sm:mt-[100px] mt-[70px]">
        <div className={`${styles.paddingX} max-w-7xl mx-auto flex justify-between`}>
          ...
          <h2 className={`${styles.sectionHeadText} `}>Forex Plan.</h2>
          </div>
...
<div className='max-w-7xl mx-auto flex justify-center items-center flex-col md:flex-row flex-wrap mt-[2rem] mb-20'>
    {ForexPlans.map(data=>{
      return(
        ...
        <Link to="/register" className='text-yellow-500 font-bold float-right border border-yellow-500 rounded-full px-4  mt-6'>Get Started</Link>
        </div>
      )
    })}
</div>
