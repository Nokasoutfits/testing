...
import { StockPlans } from '../../constants/InvestmentData'
const StockPlan = () => {
  return (
    <div>
      <HelmetProvider>
      <Helmet>
        <title>Stock Plan - SwizzFunds</title>
    ...
    <div className="black-gradient  py-10 sm:py-20 sm:mt-[100px] mt-[70px]">
        <div className={`${styles.paddingX} max-w-7xl mx-auto flex justify-between`}>
          ...
          <h2 className={`${styles.sectionHeadText} `}>Stock Plan.</h2>
          </div>
...
<div className='max-w-7xl mx-auto flex justify-center items-center flex-col md:flex-row flex-wrap mt-[2rem] mb-20'>
    {StockPlans.map(data=>{
      return(
        ...
        <Link to="/register" className='text-yellow-500 font-bold float-right  mt-6 border border-yellow-500 rounded-full px-4'>Get Started</Link>
        </div>
      )
    })}
</div>
